...
// 8. RENDER TEAM DASHBOARD (แผงควบคุมทีม)
function renderDashboard() {
    const allLogs = getLogs();
    const logs = allLogs.filter(log => !log.clearedFromDaily && isLogToday(log.timestamp));
    
    // วาดกราฟประวัติย้อนหลังด้วย
...
// ==========================================================================
// 27. HISTORICAL TREND CHART (Chart.js)
// ==========================================================================
let historicalChart = null;
let currentTrendPeriod = "30days"; // "30days" or "alltime"
let currentTrendDept = "all";

function initHistoricalChart() {
    const btn30 = document.getElementById("trend30DaysBtn");
    const btnAll = document.getElementById("trendAllTimeBtn");
    const selectDept = document.getElementById("trendDeptFilter");
    
    if (btn30) btn30.addEventListener("click", () => {
        currentTrendPeriod = "30days";
        renderTrendChart();
    });
    if (btnAll) btnAll.addEventListener("click", () => {
        currentTrendPeriod = "alltime";
        renderTrendChart();
    });
    if (selectDept) selectDept.addEventListener("change", (e) => {
        currentTrendDept = e.target.value;
        renderTrendChart();
    });
}
...
function renderTrendChart() {
    ...
    const btnAll = document.getElementById("trendAllTimeBtn");
    if (btn30) {
        btn30.className = currentTrendPeriod === "30days" ? "cozy-btn primary" : "cozy-btn secondary";
    }
    if (btnAll) {
        btnAll.className = currentTrendPeriod === "alltime" ? "cozy-btn primary" : "cozy-btn secondary";
    }
    ...
    if (currentTrendDept !== "all") {
        logsToProcess = logsToProcess.filter(l => l.memberDept === currentTrendDept);
    }
    ...
    logsToProcess.forEach(log => {
        ...
        // Avoid timezone shift issues by extracting local year, month, date
        ...
        if (!groupedByDate[dateStr]) {
            groupedByDate[dateStr] = { heartSum: 0, fireSum: 0, count: 0 };
        }
        groupedByDate[dateStr].heartSum += log.heartIndex;
        groupedByDate[dateStr].fireSum += log.fireIndex;
    ...
    // Filter by period (30 days)
    if (currentTrendPeriod === "30days") {
    ...
    sortedDates.forEach(dateStr => {
        ...
        heartData.push(Math.round(group.heartSum / group.count));
        fireData.push(Math.round(group.fireSum / group.count));
    });
